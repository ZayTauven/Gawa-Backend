...
def _resource_school_q(school):
    return Q(chapter__course__classroom__school=school) | Q(
...
class ResourceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if role == ROLE_STUDENT:
            published = school_qs.filter(status="UNLOCKED")
            return school_qs.filter(id__in=_resource_ids_for_audience(published, "STUDENT"))
        if role == ROLE_PARENT:
            published = school_qs.filter(status="UNLOCKED")
            return school_qs.filter(id__in=_resource_ids_for_audience(published, "PARENT"))
        return school_qs
    ...
    def perform_create(self, serializer):
        ...
        if chapter is None:
            raise PermissionDenied("Le chapitre est requis.")
        ...
        if getattr(user, "role", None) == ROLE_TEACHER and chapter.course.teacher_id != user.id:
